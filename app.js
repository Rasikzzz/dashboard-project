...
// Clean inspector name (remove "value" prefix)
function cleanInspectorName(name) {
    return name.replace(/^value/, '');
}

// Neon green colour for charts
...
function renderSidebar() {
    ...
    // Inspectors from leaderboard (cleaned names)
    const sorted = DATA.leaderboard.slice();
    sorted.forEach(item => {
...
function renderOverall() {
    ...
    const stateKeys = Object.keys(g.top_states);
    const stateVals = Object.values(g.top_states);
